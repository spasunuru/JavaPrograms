...
public class FindingWeight {

	public static void main(String[] args) {
		FindingWeight findingWeight = new FindingWeight();
		...
		try {
			weight = findingWeight.findWeight(i, j);
	...
	private int findWeight(int i, int j) throws GridNotSupportedException {
		int weight = 0;
		int row = 8, column =8;
		char[][] grid = new char[row][column];
		assignValue(grid);
		printChar(i, j,grid);
		weight =find(i,j, grid);
		return weight;
	}

	private int find(int x, int y, char[][] grid) throws GridNotSupportedException {
		int weight = 0;
	...
	private void printChar(int x, int y, char[][] grid) {
	...
	private void assignValue(char[][] grid) {
		for (int i =0; i<grid.length; i++)
			for(int j=0; j<grid.length; j++)
				grid[i][j] = random();
	}
